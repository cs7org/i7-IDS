...
class DataSet(torch.utils.data.Dataset):
    def __init__(
        ...
        print(f"Loading data from {input_dir / adversarial_type / data_split}")
        self.npz_files = list(input_dir.rglob(f"{adversarial_type}/{data_split}/*.npz"))
        logger.info(
            f"Found {self.npz_files}"
        )
        ...
        self.adversarial = []
        self.labels = []
        npz_data = np.load(self.npz_files[0], allow_pickle=True)
        self.inputs = npz_data["inputs"]
        self.adversarial = npz_data["adversarial"]
    ...
    def __len__(self):
        return (
            ...
            else min(len(self.inputs), self.max_samples)
        )

    def __getitem__(self, idx):
        input_image = self.inputs[idx]
        adversarial_image = self.adversarial[idx]
        ...
        img_tensor = torch.tensor(input_image, dtype=torch.float32)
        adv_tensor = (
            torch.tensor(adversarial_image, dtype=torch.float32)
        )
...
parser.add_argument(
    "--data_dir",
    type=str,
    default="/home/hpc/iwi7/iwi7101h/i7-IDS/results/adversarial_attacks/original_cnn",
...
parser.add_argument(
    "--output_dir",
    type=str,
    default="/home/hpc/iwi7/iwi7101h/i7-IDS/results/adversarial_blocking_tabular",
...
adversarial_names = [a.name for a in data_dir.iterdir() if a.is_dir()]
...
def get_predictions(
    ...
    with torch.no_grad():
        ...
        recon_input,_ = ae_model(input_tensor)
        recon_adversarial,_ = ae_model(adversarial_tensor)
...
clf_adv_ae_models = [
    ...
    [
        Path("tabular_classification/original_cnn"),
        Path("tabular_classification/original_adv_cnn"),
        Path("session_ae_experiment/ddsa_ffnn"),
    ],
    [
        Path("tabular_classification/original_fnn"),
        Path("tabular_classification/original_adv_cnn"),
        Path("session_ae_experiment/ddsa_ffnn"),
    ],
...
for adversarial_name in adversarial_names:
    if '.png' in adversarial_name or '.jpg' in adversarial_name:
        logger.warning(f"Skipping {adversarial_name} as it is not a valid adversarial type.")
        continue
    skip_this = False
    ...
    for clf_path, adv_path, ae_path in clf_adv_ae_models:
        ...
        # print(f"True targets: {true_targets}")
        # print(f"Clean CLF predictions: {clean_clf_predictions}")
