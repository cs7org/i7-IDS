    ...
    * When the feature has a negative effect, importance is < 0.
    ...
    def fit(self):
        ...
        if self.classwise:
            ...
            for label in all_labels:
                ...
                try:
                    ...
                    logger.error(f"Error fitting model for label {label}: {e}")
                    continue
        ...
        try:
            ...
            logger.error(f"Error fitting model for all labels: {e}")
    ...
    def plot(
        ...
        """
        Plot the results using Seaborn with error bars, annotate bars with importance values, and add a grid.
        """
        ...
        for label in results_df["label"].unique():
            ...
            label_df = label_df.sort_values(by="importance", ascending=True)
            ...
            sns.barplot(
                ...
                ax=ax,
                order=label_df["feature"],  # Explicitly set the order of features
            )
            ...
            # Annotate bars with importance values (3 decimals, offset to avoid error bars)
            for i, (importance, feature) in enumerate(
                zip(label_df["importance"], label_df["feature"])
            ):
                ax.text(
                    importance + 0.05,  # Offset to the right of the error bar
                    i,  # Vertical position (y-coordinate)
                    f"  {importance:.3f}",  # Format the importance value to 3 decimal places
            ...
            ax.set_ylim(-0.5, len(label_df["feature"]) - 0.5)
